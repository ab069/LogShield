...
async def register_user(db: AsyncSession, data: UserCreate) -> TokenResponse:
    ...
    if result.scalar_one_or_none(): raise ValueError("Email already registered")
...
async def login_user(db: AsyncSession, email: str, password: str) -> TokenResponse:
    ...
    if not user or not verify_password(password, user.password_hash): raise ValueError("Invalid credentials")
    return TokenResponse(access_token=create_access_token(user.id), user=UserResponse.model_validate(user))
